...
class FCViewController:UIViewController, UINavigationControllerDelegate {
    ...
    var privateMessageID:[String] = [];
    public var chatType:Constants.ChatType = .Private
    public var privateChateMemberEmail = ""
    public var  publicChannel = ""
     // MARK: Outlets
    
    @IBOutlet weak var toolbarTitle: UINavigationItem!
    public var toolbarTitleText = ""
    ...
    func setMessageId(){
        var email = (Auth.auth().currentUser?.email)!.replacingOccurrences(of: ".", with: "_")
        email = email.lowercased()
        privateChateMemberEmail = privateChateMemberEmail.lowercased()
        var list = [email, privateChateMemberEmail]
        list.sort()
        privateMessageID = list
    }
    ...
    override func viewDidLoad() {
        ...
        configureAuth()
        toolbarTitle.title = toolbarTitleText
        //print("zzz \(chatType) \(privateChateMemberEmail)")
    }
    
    override func viewDidAppear(_ animated: Bool) {
        super.viewWillAppear(true)
        addUserToRegisteredInDB()
    }
    ...
    func configureDatabase() {
        ...
        //.child((user?.uid)!).child("sentPrivate").childByAutoId()
       // _refHandle = ref.child("posts").child("messages").child((user?.uid)!).child("sentPrivate").observe(.childAdded) { (snapshot: DataSnapshot)in
        /*
        _refHandle = ref.child("messages").child(privateMessageID[0]).child(privateMessageID[1]).observe(.childAdded) { (snapshot: DataSnapshot)in
        ...
        if chatType == .Private{
            ref.child("messages").child(privateMessageID[0]).child(privateMessageID[1]).observe(.childAdded) { (snapshot: DataSnapshot)in
                self.messages.append(snapshot)
                self.messagesTable.insertRows(at: [IndexPath(row: self.messages.count-1, section: 0)], with: .automatic)
                self.scrollToBottomMessage()
            ...
            ref.child("PublicChannels").child(publicChannel).observe(.childAdded) { (snapshot: DataSnapshot)in
                self.messages.append(snapshot)
                self.messagesTable.insertRows(at: [IndexPath(row: self.messages.count-1, section: 0)], with: .automatic)
                self.scrollToBottomMessage()
        }
    ...
    func sendMessage(data: [String:Any]) {
        ...
        if chatType == .Private{
        ref.child("messages").child(privateMessageID[0]).child(privateMessageID[1]).childByAutoId().setValue(mdata)
        }
        else{
            ref.child("PublicChannels").child(publicChannel).childByAutoId().setValue(mdata)
        }
    }
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // if image message, then grab image and display it
            ...
            cell!.textLabel?.text = (name as! String) + ": " + (text as! String)
            cell!.imageView?.image = placeholderImage
        
        if (cell.textLabel?.text!)!.components(separatedBy: ":")[0] == privateChateMemberEmail.components(separatedBy: "@")[0]
        {
